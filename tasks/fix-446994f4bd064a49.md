# fix-446994f4bd064a49 — Fix: Failing dialog spec: /list when no counters exist shows empty message
...
The Tests-gate replay harness reported spec "/list when no counters exist shows empty message" as failing. 2 spec(s) failed: [/list when no counters exist shows empty message /list shows counters in alphabetical order]

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-446994f4bd064a49.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-446994f4bd064a49.json` (and any new command `tests/commands/fix-446994f4bd064a49.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-446994f4bd064a49.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-446994f4bd064a49.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
