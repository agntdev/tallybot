# fix-a353fde695386653 — Duplicate /new handler (E1T1.ts + E2T3.ts) with incompatible storage backends

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 TALLY

`src/handlers/E1T1.ts` and `src/handlers/E2T3.ts` both register `composer.command("new", …)`. Both composers run for every `/new` message, producing **double replies** AND writing data to **two completely different storage namespaces**:

- E1T1 writes to `counter:<name>` keys via `counterStore` (global, no user scope).
- E2T3 writes to `counter:<userId>:<name>` keys via `stores/counter.ts` (per-user scoped).

The two storage instances are also separate objects — `counter-store.ts` and `stores/counter.ts` each call `resolveSessionStorage()` independently, creating distinct `MemorySessionStorage` Maps in dev mode without REDIS_URL.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-a353fde695386653.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-a353fde695386653.json` (and any new command `tests/commands/fix-a353fde695386653.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-a353fde695386653.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-a353fde695386653.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
