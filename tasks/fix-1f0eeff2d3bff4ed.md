# fix-1f0eeff2d3bff4ed — E1T3 uses a separate storage instance from E1T1/E1T5, breaking /get in default dev mode

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 TALLY

E1T3 imports `getCounter` from `src/stores/counter.ts`, which lazily creates its own `resolveSessionStorage<CounterEntry>(undefined)` instance. E1T1 and E1T5 import `counterStore` from `src/counter-store.ts`, which creates a **different** `resolveSessionStorage<Counter>(undefined, process.env)` instance. Without `REDIS_URL` set (the default development mode), `resolveSessionStorage` returns `new MemorySessionStorage()` each call — giving **different `Map` instances**. A counter created by `/new` is stored in one Map; `/get` reads from a different Map and returns "not found." With REDIS_URL both happen to use the same key prefix (`sess:counter:`) so data **is** shared in production — but the two separate adapter instances are still wasteful and confusing.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-1f0eeff2d3bff4ed.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-1f0eeff2d3bff4ed.json` (and any new command `tests/commands/fix-1f0eeff2d3bff4ed.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-1f0eeff2d3bff4ed.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-1f0eeff2d3bff4ed.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
