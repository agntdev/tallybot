# fix-7ccc3a02c4b54bfe — Race condition in counter limit enforcement — check-then-act not atomic

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 TALLY

`createUserCounter` in `src/stores/counter.ts` calls `countUserCounters(userId)` then `s.write(k, {value: 0})` as two separate, non-atomic operations. Under concurrent `/new` requests, two callers can both read a count below `MAX_COUNTERS_PER_USER` (1000) and both write, resulting in 1001+ counters — exceeding the enforced limit that the task spec (`E2T3.md`) explicitly requires. The fix would be a Redis `WATCH`/`MULTI`/`EXEC` transaction or a Lua script that atomically checks-and-sets.

Relevant lines: `src/stores/counter.ts:138-146` — `countUserCounters` at 138, conditional at 139, write at 146.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-7ccc3a02c4b54bfe.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-7ccc3a02c4b54bfe.json` (and any new command `tests/commands/fix-7ccc3a02c4b54bfe.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-7ccc3a02c4b54bfe.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-7ccc3a02c4b54bfe.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
