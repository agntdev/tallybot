# fix-24693b9c45216239 — Duplicate /help handler (T01.ts + T03.ts) causes double-reply at runtime

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 TALLY

`src/handlers/T01.ts` and `src/handlers/T03.ts` both register `composer.command("help", …)` with identical reply text. grammy middleware is additive — when two composers match the same `/help` command, **both handlers fire** and the user receives two identical replies. The task specs for T01 (verify boot) and T03 (implement /help) overlapped without coordination.

The grammy composer loader processes `T01.ts` before `T03.ts` alphabetically, but both middlewares run. A user sending `/help` gets the help message duplicated.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-24693b9c45216239.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-24693b9c45216239.json` (and any new command `tests/commands/fix-24693b9c45216239.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-24693b9c45216239.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-24693b9c45216239.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
